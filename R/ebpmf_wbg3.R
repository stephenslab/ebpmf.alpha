...
ebpmf_wbg3 <- function(X, K,
  ...
  ## initialization
	init_tmp <- init_ebpmf_wbg3(X = X, K = K, init = init, d = d, seed = seed)
  qg <- init_tmp$qg
  b <- init_tmp$b
  a <- init_tmp$a
	l0 <- init_tmp$l0
  ...
  for(i in 1:maxiter){
      ...
      rank1_qg <- rank1_wbg(d = d, X_rs = Ez$rs, X_cs = Ez$cs,
														l0 = l0$mean, f0 = f0$mean, w_log_k = w_log[k],
      ...
      b_k = w_log[k] + qg$qls_mean_log[d$i,k] + qg$qfs_mean_log[d$j, k] - a
		...
		for(k in 1:K){
			b_k = w_log[k] + qg$qls_mean_log[d$i,k] + qg$qfs_mean_log[d$j, k] - a
			w_log[k] = log( sum(d$x * exp(b_k - b)) ) - log( sum(l0$mean * qg$qls_mean[,k]) * sum(f0$mean * qg$qfs_mean[,k]) )
	...
	## update l0, f0
    if(!fix_option$l0){
      s = colSums(exp(w_log) * t(qg$qls_mean) * colSums(f0$mean * qg$qfs_mean))
      l0_tmp <- do.call(pm_func$l0, c(list(x = X_rs, s = s, g_init = l0$g), pm_control))
      l0$mean <- l0_tmp$posterior$mean
      l0$mean_log <- l0_tmp$posterior$mean_log
      l0$g <- l0_tmp$fitted_g
      l0$kl <- compute_kl_ebpm(y = X_rs, s = s,
                               posterior = l0_tmp$posterior,
                               ll = l0_tmp$log_likelihood)
      rm(l0_tmp)
    }
    if(!fix_option$f0){
      s = colSums(exp(w_log) * t(qg$qfs_mean) * colSums(l0$mean * qg$qls_mean))
      ...
      rm(f0_tmp)
		...
		#browser()
		ELBO = compute_elbo_wbg3(w = w, l0 = l0, f0 = f0, qg = qg,
